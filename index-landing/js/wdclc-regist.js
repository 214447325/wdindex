    ...
    $choose.click(function(event) {
        $invitation.slideToggle(function(){
            if ($invitation.is(':hidden')) {
                $choose.css('background-image', 'url(../images/la.png)');
            }else{
                $choose.css('background-image', 'url(../images/xia.png)');
            }
        });
    });
...
//              alert("checkForm()的返回值"+checkForm());
        if(checkForm()){
            ...
            $.ajax({
                ...
                if(res.code==1){
                    window.location.href = '../pages/landing-success.html';
                }else{
                    alert(res.message);
                    $this.removeClass('disabled');
                    return false;
